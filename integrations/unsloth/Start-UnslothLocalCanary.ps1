...
$allowedModels = @($manifest.models | Where-Object {
		...
		$_.state -notin @('disabled', 'retired')
})
if ($allowedModels.Count -ne 1) {
	throw "Model '$Model' is not available in the canary deployment."
}
...
# Unsloth persists its supported external-provider selection in the Studio UI.
# This launcher deliberately leaves private application state untouched.
# Configure a Custom provider for the v2 endpoint once, then select it there.
